...
public class Merge2BST {
	...
	// The function to print data of two BSTs in sorted
	// order
	static void merge(Node root1, Node root2)
	{
		...
		// Run the loop while there are nodes not yet
		// printed. The nodes may be in stack(explored, but
		...
		while (current1 != null || !s1.isEmpty()
			...
			if (current1 != null || current2 != null) {
				// Reach the leftmost node of both BSTs and
				// push ancestors of leftmost nodes to stack
				...
				// stacks is empty, then one tree is
				// exhausted, print the other tree
				if (s1.isEmpty()) {
					while (!s2.isEmpty()) {
						current2 = s2.pop();
						current2.left = null;
				...
				// Pop an element from both stacks and
				...
				// print it and push the right subtree. If
				// the element is larger, then we push it
				// back to the corresponding stack.
